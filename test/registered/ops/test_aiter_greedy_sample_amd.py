...
def _mock_global_server_args(backend="pytorch"):
    ...
    # `Sampler.__init__` asks the context for the group; state one, since this
    # process has no distributed init. Left entered for the test's lifetime.
    get_parallel().override(tp_group=_DummyTPGroup()).__enter__()
